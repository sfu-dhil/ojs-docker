...
docker exec ojs_app php tools/importExport.php PodcastImportPlugin --journal-path=j1 --rss-url=<RSS FEED URL> --add-contributor-roles
```
...
<!--
# Run Tests (not working at the moment)
...
Deps:
- Install XQuartz (`brew cask install xquartz`) for to allow Cypress to open its GUI while running in Docker (for MacOS). **Remember** to logout after installing

```bash
open -a XQuartz
```

`XQuartz` -> `Settings` -> `Security` -> Make sure that `Allow connections from network clients` is checked

### Setup OJS test env

Add j1.localhost to your hosts file (if needed)
```bash
sudo nano /etc/hosts
#add the following to the end of the file
127.0.0.1 j1.localhost
...
```bash
# create persistence dir
mkdir -p .data/test_app

# get the `config.inc.php` from `config.TEMPLATE.inc.php` and copy over the plugin directory as starting base
docker create --name=ojs_config_temp --platform=linux/amd64 pkpofficial/ojs:3_5_0-1
docker cp ojs_config_temp:/var/www/html/config.TEMPLATE.inc.php .data/test_app/config.inc.php
docker rm ojs_config_temp

# update the `config.inc.php` with mailhog settings
sed -i '' -e "s/default = sendmail/default = smtp/g" .data/test_app/config.inc.php
sed -i '' -e "s/sendmail_path =/; sendmail_path =/g" .data/test_app/config.inc.php
sed -i '' -e "s/; smtp = On/smtp = On/g" .data/test_app/config.inc.php
sed -i '' -e "s/; smtp_server = .*/smtp_server = test_mail/g" .data/test_app/config.inc.php
sed -i '' -e "s/; smtp_port = 25/smtp_port = 1025/g" .data/test_app/config.inc.php

# update the `config.inc.php` with some config secrets
sed -i '' -e "s/^salt = .*$/salt = \"$(openssl rand -hex 48 | tr -d '\n')\"/g" .data/test_app/config.inc.php
sed -i '' -e "s/^api_key_secret = .*$/api_key_secret = \"$(openssl rand -hex 48 | tr -d '\n')\"/g" .data/test_app/config.inc.php
sed -i '' -e "s/require_validation = Off/require_validation = On/g" .data/test_app/config.inc.php
```

Now startup the OJS container for the first time along with init the journal `Journal 1`

```bash
docker compose -f docker-compose.cypress.yaml up -d --build db app mail
# wait a few seconds for the server to fully startup

# basic site install
BASE_URL=http://j1.localhost:8080 DOMAIN=j1.localhost DATABASE_HOST=db DATABASE_USER=test_ojs DATABASE_NAME=test_ojs DATABASE_PASSWORD=password ADMIN_EMAIL=dhil@sfu.ca ADMIN_PASSWORD=password scripts/cli-install-site.sh

# journal install + some misc (turn off admin notifications)
BASE_URL=http://j1.localhost:8080 JOURNAL_ACRONYM=j1 JOURNAL_NAME="Journal 1" SITE_TITLE="Test OJS Instance" CONTACT_NAME="OJS Administrator" CONTACT_EMAIL=dhil@sfu.ca ADMIN_PASSWORD=password scripts/cli-install-journal.sh

# fix base url
docker exec -e BASE_URL=http://j1.localhost:8080 test_ojs_app /usr/local/bin/cli-fix-config-base-url

# turn off install for tests
sed -i '' -e "s/installed = On/installed = Off/g" .data/test_app/config.inc.php
```

## Start test docker containers

```bash
# make sure to leave the XQuartz application running
/usr/X11/bin/xhost + $(ipconfig getifaddr en0)
IP=$(ipconfig getifaddr en0) docker compose -f docker-compose.cypress.yaml up -d --build
```
> Note: make sure the regular environment is not already running first (`docker compose down`).

### cleanup

```bash
IP=$(ipconfig getifaddr en0) docker compose -f docker-compose.cypress.yaml down
```
